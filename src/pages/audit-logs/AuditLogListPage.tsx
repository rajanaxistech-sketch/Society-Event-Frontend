  ...
  const columns: Column<AuditLogItem>[] = [
    {
      ...
      header: 'Event Action',
      render: (row) => (
        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase font-mono ${getActionColor(row.action)}`}>
          {row.action}
        </span>
      ),
    },
    ...
    },
    {
      ...
      render: (row) => (
        <div className="text-xs">
          ...
          <span className="text-slate-400 text-[11px]">{row.user?.email || row.ip_address || ''}</span>
        </div>
      ),
    },
    {
  ...
  return (
    <div className="space-y-3 sm:space-y-3.5">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5">
        <div>
          <h1 className="text-lg sm:text-xl font-bold text-slate-900 tracking-tight">Security Audit Logs</h1>
